...
private String createSRdbUrl() {
 ...
 public void createConnectionSRdb() {
   ...
   public String getMyThread() {
    ...
    /* qerror = "update _rdb_bdr.tc_process set n_operation ='shutdown' , n_dateop= now() where n_pid= " +threadn + " and n_mstr = '" +nodemst+ "' and  n_type = 'C';" ;
       in caso di errore sul Coordinator spegne tutto
     */

         qerror = "update _rdb_bdr.tc_process set n_operation ='shutdown' , n_dateop= now() ;" ;
 ...
 public boolean isSlotActive(Connection connection, int itxid)
    ...
    public  int refreshCoorPast() throws SQLException, Exception {
        ...
        LOGGERM.trace( mythread+":" +"Coordinate producer " + nodemst + " past");
        if (checkMasterLsnPast(true,nodemst)) {


        } else {
            LOGGERM.trace( mythread+":" +"Coordinate producer " + nodemst+ " not need to manage LSN table partitions");
        }
        ...
        try (PreparedStatement preparedStatement =  connectionRdb.prepareStatement("select substr(slot_name, length('"+nodemst+"_publ_')+1) from pg_replication_slots where slot_name like '"+nodemst+"%' and  slot_name not like '"+nodemst+"%sync%'")) {
            try (ResultSet rs = preparedStatement.executeQuery()) {
                while (rs.next()) {
                    LOGGERM.trace( mythread+":" +"Coordinate remote consumer " + rs.getString(1)  + " for  producer " +  nodemst );
                    scanned = scanned + 1;
                    if  (checkMasterLsnPast(false,(rs.getString(1) )) ) {

                    } else {
                        LOGGERM.trace( mythread+":" +"Coordinate producer remote consumer " + rs.getString(1)  + " for  producer " +  nodemst+ " not need to manage LSN table partitions");
                    }
    ...
    public boolean checkMasterLsnPast(boolean isMaster,String aSlave) throws SQLException, Exception  {
        ...
        try {
                ...
                if (!isMaster) {
                    ...
                    try (PreparedStatement preparedStatement = connectionSRdb.prepareStatement(qq2)) {
                        try (ResultSet rs = preparedStatement.executeQuery()) {
                            if (null != rs && rs.next())
                                if (  !rs.getBoolean(1)){
                                    LOGGERM.trace( mythread+":" +"Coordinate consumer " +aSlave+ " slave of "+ nodemst +" - Past WAL LSN:" + srs.getString(1) +"- NOT EXIST" );
                                } else {
                                    LOGGERM.trace( mythread+":" +"Coordinate consumer " +aSlave+ " slave of "+ nodemst +" - Past WAL LSN:" + srs.getString(1) +"-  EXIST" );
                    ...
                    try (PreparedStatement preparedStatement = connectionRdb.prepareStatement(qq2)) {
                        try (ResultSet rs = preparedStatement.executeQuery()) {
                            if (null != rs && rs.next())
                                if (!rs.getBoolean(1)) {
                                    LOGGERM.trace(mythread + ":" + "Coordinate producer " + nodemst + " - Past WAL LSN:" + srs.getString(1) + "- NOT EXIST");
                                } else {
                                    LOGGERM.trace(mythread + ":" + "Coordinate producer " + nodemst + " - Past WAL LSN:" + srs.getString(1) + "-  EXIST");
            ...
            if (missingLSN && !isMaster) {
                try {
                    Statement st = connectionSRdb.createStatement();
                    LOGGERM.trace( mythread+":" +"Coordinate consumer " +aSlave+ " slave of "+ nodemst +" - Refresh publication " );
    ...
    public  int refreshCoor() throws SQLException, Exception {
        ...
        LOGGERM.trace( mythread+":" +"Coordinate producer " + nodemst);
            ...
            if (!isMaster) {
                ...
                try (PreparedStatement preparedStatement = connectionSRdb.prepareStatement(qq2)) {
                    try (ResultSet rs = preparedStatement.executeQuery()) {
                        if (null != rs && rs.next())
                            if (  rs.getBoolean(1)){
                                LOGGERM.trace( mythread+":" +"Coordinate consumer " +aSlave+ " slave of "+ nodemst +" - Current WAL LSN:" + srs.getString(1) +"- EXIST" );
                            } else {
                                LOGGERM.trace( mythread+":" +"Coordinate consumer " +aSlave+ " slave of "+ nodemst +" - Current WAL LSN:" + srs.getString(1) +"- NOT EXIST" );
                                ...
                                try {
                                    ...
                                    st.execute("CREATE INDEX walq__"+nodemst +"_"+srs.getString(1)+"_idx2 ON _rdb_bdr.walq__"+nodemst +"_"+srs.getString(1)+" USING btree (dateop)");
                                    st.execute("CREATE INDEX walq__"+nodemst +"_"+srs.getString(1)+"_idx3 ON _rdb_bdr.walq__"+nodemst +"_"+srs.getString(1)+" USING btree (dateop,wid )");
                                    st.execute("CREATE INDEX walq__"+nodemst +"_"+srs.getString(1)+"_idx4 ON _rdb_bdr.walq__"+nodemst +"_"+srs.getString(1)+" USING btree (local_xid )");
                                    st.close();
                                } catch (SQLException e) {
                                    e.printStackTrace();
                                }
                ...
                try (PreparedStatement preparedStatement = connectionRdb.prepareStatement(qq2)) {
                    try (ResultSet rs = preparedStatement.executeQuery()) {
                        if (null != rs && rs.next())
                            if (rs.getBoolean(1)) {
                                LOGGERM.trace(mythread + ":" + "Coordinate producer " + nodemst + " - Current WAL LSN:" + srs.getString(1) + "- EXIST");
                            } else {
                                LOGGERM.trace(mythread + ":" + "Coordinate producer " + nodemst + " - Current WAL LSN:" + srs.getString(1) + "- NOT EXIST");
 ...
 public synchronized void loadSlaveProps(String nodeslv) {
   ...
   if(!Files.isRegularFile(Paths.get(nodeslv_rdbbdr_conf))) {
    ...
    try (InputStream input = new FileInputStream(nodeslv_rdbbdr_conf)) {
            ...
            prop.load(input);
            LOGGERM.trace(mythread+ ": reading config file for consumer node "+nodeslv);
