...
private boolean initialSetupCons()  {
  ...
  //  String qq= " insert into _rdb_bdr.tc_process (n_id,n_name,n_shouldbe,n_state,n_operation,n_type,n_mstr,n_dateop,n_datecrea,n_pid) values (0,'" +node + "', 'up','stop','managed','S','"+primary+"', now(),now(),-1)  ON CONFLICT ON CONSTRAINT tc_process_pkey DO NOTHING;";
	...
	String qqO = "insert into _rdb_bdr.walq__"+primary+"_offset (src_topic_id,last_offset,xid_offset,lsn_offset,dateop,local_xid_offset) values ('"+node+"',0,0, "+ (((BaseConnection) connection).haveMinimumServerVersion(ServerVersion.v10) ? "pg_current_wal_lsn()" : "pg_current_xlog_location()") +",current_timestamp,0)  ON CONFLICT ON CONSTRAINT walq__"+primary+"_offset_pkey DO NOTHING;";
