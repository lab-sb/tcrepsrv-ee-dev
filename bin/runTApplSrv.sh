...
if [ -z "$NODEMST" ]; then
    echo "NODEMST not specify, use runTApplSrv.sh -n nodemst -s nodeslv"
    exit 0
fi

if [ -z "$NODESLV" ]; then
    echo "NODESLV not specify, use runTApplSrv.sh -n nodemst -s nodeslv"
    exit 0
fi
...
echo " "
tail -9f $(cat ../conf/${NODESLV}_bdr_log.conf|grep "^java.util.logging.FileHandler.pattern"|awk -F '=' '{print $2}')
